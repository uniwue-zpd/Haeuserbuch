...
public class QuarterController {
    ...
    @GetMapping("/search")
    public ResponseEntity<List<QuarterDTO>> searchQuarters(@RequestParam String query) {
        if (query == null || query.trim().length() < 3) {
            return ResponseEntity.ok(List.of());
        }
        List<QuarterDTO> quarterDTOs = quarterService.searchQuarters(query);
        return ResponseEntity.ok(quarterDTOs);
    }
}
