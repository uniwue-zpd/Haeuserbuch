...
const authors = computed(() => sourceItem.value?.authors?.length ? sourceItem.value.authors : null);
const links = computed(() => sourceItem.value?.links?.length ? sourceItem.value.links : null);
    ...
    <template #title>
      <div class="flex flex-col gap-2 montserrat-headline">
        <div class="flex flex-row justify-between">
          <h1 class="text-3xl font-bold text-black max-w-[80%]">{{ sourceItem?.title }}</h1>
          <TaskBar :id="sourceId" entity_type="sources"/>
        </div>
        <h2 class="text-2xl font-semibold text-gray-600" v-show="sourceItem?.type">{{ sourceItem?.type }}</h2>
      </div>
    </template>
    <template #content>
      <Divider/>
    ...
    <template #footer>
      <div class="flex flex-col gap-2">
        <Panel header="Notizen" toggleable v-show="sourceItem?.generalNotes">
          ...
          <p class="text-sm text-black roboto-plain">{{ sourceItem?.generalNotes }}</p>
        </Panel>
        <Divider/>
        <div class="flex flex-col">
          <div v-if="sourceItem?.createdDate" class="flex flex-row space-x-2 text-sm text-black roboto-plain">
            <p class="font-bold">Erstellt am:</p>
            <p>{{ new Date(sourceItem?.createdDate).toLocaleDateString() }}</p>
          </div>
          <div v-if="sourceItem?.lastModifiedDate" class="flex flex-row space-x-2 text-sm text-black roboto-plain">
            <p class="font-bold">Stand:</p>
            <p>{{ new Date(sourceItem?.lastModifiedDate).toLocaleDateString() }}</p>
          </div>
        </div>
      </div>
    </template>
