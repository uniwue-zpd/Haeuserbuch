    ...
    <template #footer>
      <div class="flex flex-col gap-2">
        ...
        <div class="flex flex-col">
          <div
            v-if="districtItem?.createdDate"
            class="flex flex-row space-x-2 text-sm text-black"
          >
            <p>Erstellt am:</p>
            <p>
              {{ new Date(districtItem?.createdDate).toLocaleDateString() }}
            </p>
          </div>
          <div
            v-if="districtItem?.lastModifiedDate"
            class="flex flex-row space-x-2 text-sm text-black"
          >
            <p>Stand:</p>
            <p>
              {{
                new Date(districtItem?.lastModifiedDate).toLocaleDateString()
              }}
            </p>
          </div>
        </div>
