  ...
  <div v-else>
    <div v-if="citizenshipItem" class="flex flex-col gap-4 p-4 rounded-lg shadow-lg border-2 border-gray-300">
      <div class="flex flex-row justify-between">
        <h1 class="text-3xl montserrat-headline font-bold">{{ citizenshipItem?.signature }}</h1>
      ...
      <div class="flex flex-col gap-4">
        <div class="roboto-plain flex flex-col gap-5 p-4 rounded-lg shadow-lg border border-gray-300">
          <h2 class="text-2xl font-semibold montserrat-headline">Informationen zum Eintrag</h2>
          <div v-if="citizenshipItem.signature" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Signatur</p>
            <p class="font-semibold">{{ citizenshipItem.signature }}</p>
          </div>
          <div v-if="citizenshipItem.refNumber" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Referenz Meyer-Erlach</p>
            <p class="font-semibold">{{ citizenshipItem.refNumber }}</p>
          </div>
          <div v-if="citizenshipItem.dateNaturalization" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Datum der Einbürgerung</p>
            <p class="font-semibold">{{ citizenshipItem.dateNaturalization }}</p>
          </div>
          <div v-if="citizenshipItem.dateMisc" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Andere Datumsangaben</p>
            <p class="font-semibold">{{ citizenshipItem.dateMisc }}</p>
          </div>
          <div v-if="citizenshipItem.person" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Eingebürgerte Person</p>
            <PersonPreview :person="citizenshipItem.person"/>
          </div>
          <div v-if="citizenshipItem.mentionedPersons.length > 0" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Weitere erwähnte Personen</p>
          ...
          <div v-if="citizenshipItem.entryText" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Text des Eintrags</p>
            <p class="text-justify">{{ citizenshipItem.entryText }}</p>
          </div>
          <div v-if="citizenshipItem.addendum" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Nachtrag</p>
            <p class="text-justify">{{ citizenshipItem.addendum }}</p>
          </div>
        </div>
        <div
            v-if="citizenshipItem.primarySource || citizenshipItem.secondarySource"
        ...
        >
          ...
          <div v-if="citizenshipItem.primarySource" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Primärquelle</p>
            <SourcePreview :source="citizenshipItem.primarySource"/>
          </div>
          <div v-if="citizenshipItem.secondarySource" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
        ...
        <div class="roboto-plain flex flex-col gap-5 p-4 rounded-lg shadow-lg border border-gray-300">
          ...
          <div v-if="citizenshipItem.createdDate" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Erstellt am</p>
            <p class="text-justify">{{ new Date(citizenshipItem.createdDate).toLocaleDateString() }}</p>
          </div>
          <div v-if="citizenshipItem.lastModifiedDate" class="flex flex-col gap-2 border-l-4 border-gray-300 pl-3 py-1.5">
            <p class="text-sm text-gray-500 font-medium">Zuletzt aktualisiert am</p>
            <p class="text-justify">{{ new Date(citizenshipItem.lastModifiedDate).toLocaleDateString() }}</p>
          </div>
